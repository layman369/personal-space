...
首版文章、海岸图片和两个工具为明确标注的示例，不代表站主的真实经历或既有作品。海岸图片由 AI 生成。没有虚构联系方式或视频素材。
...
视频添加 `type: '视频'`，`src` 使用可公开访问的 HTTPS MP4/WebM 地址，`poster` 是封面图片路径。不要把普通视频网站的观看页当成视频源地址；它们需要各自的嵌入播放器。第一版没有默认视频，视频分类显示空状态。
